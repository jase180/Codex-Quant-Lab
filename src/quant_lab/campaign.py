...
def update_campaign_state_after_execution(
    ...
    *,
    ...
    remaining_budget["cycles"] = max(0, remaining_budget.get("cycles", 0) - 1)
    remaining_budget["runs"] = max(0, remaining_budget.get("runs", 0) - projected_run_count)
    remaining_budget["seconds"] = max(0, remaining_budget.get("seconds", 0) - execution.elapsed_seconds)
    ...
    return replace(
        ...
        do_not_repeat=_ordered_unique(
            [*state.do_not_repeat, *_text_list(conclusion.get("do_not_repeat")), *_branch_repetition_rules(conclusion)]
        ),
        unresolved_questions=_ordered_unique([*state.unresolved_questions, *_text_list(conclusion.get("open_questions"))]),
