...
def campaign_provider_result(
    ...
    *,
    ...
    if config.provider == "codex":
        return _codex_campaign_provider_result(
            config,
            state,
            cycle_dir=cycle_dir,
            prior_attempt_feedback=prior_attempt_feedback,
...
def _codex_campaign_provider_result(
    config: CampaignConfig,
    state: CampaignState,
    *,
    cycle_dir: str | Path | None,
    ...
    proposal = CampaignProposal(
        ...
        title="Codex campaign proposal handoff",
        hypothesis="A Codex campaign proposal requires an external Codex response before execution.",
        rationale=(
            "The standalone campaign CLI cannot call the current Codex chat session. "
            "It wrote provider_context.json and provider_prompt.md so Codex can review the exact bounded context."
        ),
        difference_from_prior_work="No experiment proposal was generated by an automated Codex adapter.",
