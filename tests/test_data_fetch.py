...
class DataFetchTests(unittest.TestCase):
    ...
    def test_build_market_data_provenance_records_source_and_fingerprint(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(provenance["price_adjustment"]["auto_adjust"], True)
            self.assertEqual(provenance["price_adjustment"]["actions"], False)
            self.assertIn("provider-adjusted", provenance["price_adjustment"]["note"])
