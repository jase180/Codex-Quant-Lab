...
5. The project has a real offline health path. `quant-lab doctor` and `quant-lab smoke-test --agent-cycle` are implemented and tested; the latest full suite passed `384` tests, and the real smoke command verifies agent dry-run wiring without executing proposed commands.
